...
EXCLUDES := -x ./pkg_resources/\* ./__pycache__/\* ./setuptools*/\* ./pip*/\* __pycache__
...
bundle.local:
	zip -r -9 "$(ZIP_FILE)" game *.py pycharm-debug-py3k.egg -x ./pkg_resources/* *tests*
	cd "$(VENV_DIR)/lib/python$(PY_VERSION)/site-packages" \
		&& zip -r9 "$(ZIP_FILE)" * $(EXCLUDES)
	cd "$(VENV_DIR)/lib64/python$(PY_VERSION)/site-packages" \
		&& zip -r9 "$(ZIP_FILE)" * $(EXCLUDES)

bundle:
	docker run -v $$PWD:/var/task -it lambci/lambda:build-python3.6 /bin/bash -c 'make build bundle.local'
